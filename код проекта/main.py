...
audio_folder = os.path.join(game_folder, "data/audio")
...
def on_music(type_of_music):
    if type_of_music == "menu":
        pygame.mixer.music.load(choice([f'{audio_folder}/CypherTwin.wav',
                                        f'{audio_folder}/MorphadronRezidue.wav']))
    elif type_of_music == "game":
        pygame.mixer.music.load(choice([f'{audio_folder}/CypherTwin.wav',
                                        f'{audio_folder}/MorphadronRezidue.wav']))
    elif type_of_music == "game_over":
        pygame.mixer.music.load(choice([f'{audio_folder}/a1.wav']))
    ...
    def collisions(self, x, y, objects):
        for object in objects:
            if pygame.sprite.collide_rect(self, object):  # При столкновении
                ...
                if object.is_flying and y > 0 and object.rect.y < self.rect.y:
